...
from config import ALLOWED_RECIPIENT, SMTP_SERVER, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_FROM

def send_inspection_email(recipient: str, reports: dict) -> bool:
    # 邊界檢查：強制僅允許指定收件者
    if recipient.strip().lower() != ALLOWED_RECIPIENT.lower():
        print(f"[SECURITY REJECT] 存取被拒：目前代理設定只允許寄給指定收件人 {ALLOWED_RECIPIENT}，拒絕寄送至 {recipient}")
        return False
    ...
    try:
        with smtplib.SMTP(SMTP_SERVER, SMTP_PORT) as server:
            ...
            server.sendmail(SMTP_FROM, [ALLOWED_RECIPIENT], msg.as_string())
        print(f"[MAIL SUCCESS] 巡檢報告已成功寄送至 {ALLOWED_RECIPIENT}")
        return True
